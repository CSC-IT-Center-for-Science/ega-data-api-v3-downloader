/*
 * Copyright 2016 ELIXIR EBI
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package eu.elixir.ega.ebi.downloader.domain.entity;

import java.io.Serializable;
import java.sql.Timestamp;
import javax.persistence.Column;

import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
//import javax.persistence.JoinColumn;
//import javax.persistence.ManyToOne;
import javax.persistence.Table;
import javax.validation.constraints.NotNull;
import javax.validation.constraints.Size;

import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.AllArgsConstructor;
import lombok.Setter;

/**
 *
 * @author asenf
 */
@NoArgsConstructor
@AllArgsConstructor
@Setter
@Getter
@Entity
@Table(name = "download_ticket")
public class DownloadTicket implements Serializable {

        @Id
        @NotNull
        @GeneratedValue(strategy=GenerationType.IDENTITY)
        @Column(name="download_ticket_id", nullable=false)
        private long downloadTicketId;
        
        @NotNull
        @Size(max=256)
        @Column(name="user_email", nullable=false, length=256)
        private String userEmail;
        
        @NotNull
        @Size(max=256)
        @Column(name="download_ticket", nullable=false, length=256)
        private String downloadTicket;
        
        @NotNull
        @Size(max=45)
        @Column(name="client_ip", nullable=false, length=45)
        private String clientIp;
        
        @NotNull
        @Size(max=15)
        @Column(name="file_stable_id", nullable=false, length=15)
        private String fileStableId;
        
        @NotNull
        @Column(name="encryption_key", nullable=false, columnDefinition="text")
        private String encryptionKey;
        
        @NotNull
        @Size(max=28)
        @Column(name="encryption_type", nullable=false, length=28)
        private String encryptionType;
        
        @NotNull
        @Size(max=28)
        @Column(name="ticket_status", nullable=false, length=28)
        private String ticketStatus;
        //@ManyToOne
        //@JoinColumn(name="ticket_status")
        //DownloadTicketStatusCv statusCV;
        
        @NotNull
        @Size(max=256)
        @Column(nullable=false, length=256)        
        private String label;
        
        @NotNull
        @Column(nullable=false)
        private Timestamp created;
        
        @NotNull
        @Column(name="start_coordinate", nullable=false)
        private long startCoordinate;
        
        @NotNull
        @Column(name="end_coordinate", nullable=false)
        private long endCoordinate;
                
        /*
         * 
         */
        public String toString() {
            String line = "";
            
            line += "ID: " + downloadTicketId + "\n" +
                    "User Email: " + userEmail + "\n" +
                    "Download Ticket: " + downloadTicket + "\n" +
                    "Client IP: " + clientIp + "\n" +
                    "File Stable ID: " + fileStableId + "\n" +
                    "Encryption Key: " + encryptionKey + "\n" +
                    "Encryption Type: " + encryptionType + "\n" +
                    //"Ticket Status: " + statusCV.getTicket_status() + "\n" +
                    "Ticket Status: " + ticketStatus + "\n" +
                    "Label: " + label + "\n" +
                    "Created: " + created.toString() + "\n" +
                    "Start Coordinate: " + startCoordinate + "\n" +
                    "End Coordinate: " + endCoordinate;

            return line;
        }
}
